  ...
  return (
    <Draggable draggableId={tarefa.id} index={indice}>
      {(fornecido, estadoArraste) => (
        ...
        >
          <div
            className={`relative rounded-xl border transition-all duration-200 surface-shadow surface-shadow-hover overflow-hidden ${
              estadoArraste.isDragging
            ...
            } ${estilo === 'SPIRAL' ? 'ruled-background pt-5 px-4 pb-3.5' : ''} ${
              estilo === 'TAPE' ? 'pt-4 px-4 pb-3.5' : ''
            } ${estilo === 'BINDER' ? 'py-3.5 pr-4 pl-7' : ''}`}
          >
            {estilo === 'SPIRAL' && (
              <div
                aria-hidden="true"
            ...
            <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 pt-1 border-t border-slate-100 dark:border-slate-700/50">
              <div className="flex items-center gap-2.5">
                ...
                <div
                  title={`${tarefa.quantidadeComentarios ?? 0} comentários`}
                  className="flex items-center gap-1 text-slate-500 dark:text-slate-400 font-medium"
                >
                  <MessageSquare className="w-3.5 h-3.5" />
                  <span className="text-[11px]">{tarefa.quantidadeComentarios ?? 0}</span>
                </div>
              </div>
